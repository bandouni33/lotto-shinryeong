...
const SWIFT_KAKAO_LINKING_STRING = 'if kakao_login.RNKakaoLogins.isKakaoTalkLoginUrl(url) { return kakao_login.RNKakaoLogins.handleOpen(url) }';
...
const modifyInfoPlist = (config, props) => {
    return (0, config_plugins_1.withInfoPlist)(config, async (config) => {
        ...
        if (!Array.isArray(config.modResults.LSApplicationQueriesSchemes)) {
            ...
            KAKAO_SCHEMES.forEach((scheme) => !config.modResults.LSApplicationQueriesSchemes?.includes(scheme) &&
...
const modifyAppDelegate = (config) => {
    ...
    const modifySwiftContents = (contents) => {
        ...
        if (!contents.includes(SWIFT_KAKAO_LINKING_STRING)) {
            contents = (0, codeMod_1.insertContentsInsideSwiftFunctionBlock)(contents, 'application(_:open:options:)', SWIFT_KAKAO_LINKING_STRING, { position: 'head' });
        }
        return contents;
    };
    return (0, config_plugins_1.withAppDelegate)(config, (props) => {
        if (['objc', 'objcpp'].includes(props.modResults.language)) {
            ...
            props.modResults.contents = modifySwiftContents(props.modResults.contents);
        }
        return props;
    });
};
