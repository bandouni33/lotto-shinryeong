...
def render():
    ...
    st.markdown("""
        ...
        div[data-testid="stHorizontalBlock"]:has(div[class*="st-key-th_nav_home"]) {
            flex-wrap: nowrap !important;
            gap: 10px !important;
            margin-bottom: 12px !important;
        }
        div[data-testid="stHorizontalBlock"]:has(div[class*="st-key-th_nav_home"]) > div[data-testid="column"] {
            min-width: 0 !important;
            flex: 1 1 0 !important;
        }
        div[class*="st-key-th_nav_home"] button,
        div[class*="st-key-th_nav_bday"] button {
    ...
    n1, n2 = st.columns(2)
    with n1:
        if st.button("⬅️", use_container_width=True, key="th_nav_home"):
            st.query_params.clear()
            st.rerun()
    with n2:
        if st.button("📝 생일/행운수 관리", use_container_width=True, key="th_nav_bday"):
            st.query_params.clear()
        ...
        <script>
            ...
            function initGrid() {{
                ...
                for(let i=1; i<=45; i++) {{
                    ...
                    if(luckyLoaded && luckyNumbers.has(i)) cell.classList.add('selected-lucky');
                    cell.innerText = i;
